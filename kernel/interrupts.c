...
static uint16_t pic_get_isr(void) {
    outb(PIC1, 0x0B); outb(PIC2, 0x0B); //OCW3: select the in-service register
    return ((uint16_t)inb(PIC2) << 8) | inb(PIC1);
}

static void (*tick_hook)(void);
void irq_set_tick_hook(void (*fn)(void)) { tick_hook = fn; }

void irq_dispatch(uint64_t irq) {
    if (irq == 7 && !(pic_get_isr() & (1 << 7))) return; //spurious: no EOI
    if (irq == 15 && !(pic_get_isr() & (1 << 15))) { outb(PIC1, 0x20); return; } //EOI master only
    if (irq < 16 && handlers[irq]) handlers[irq]();
    pic_eoi((int)irq);
    if (irq == 0 && tick_hook) tick_hook(); //preemptive schedule, after EOI
}